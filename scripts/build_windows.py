...
from src_artifact.utils import contains_external_marker, load_yaml, validate_common_config
...
def main() -> int:
    ...
    paths = cfg.get("paths", {})
    if any(contains_external_marker(v) for v in paths.values()):
        print("External input tables are not included in this repository.")
    summary = {
        "status": "dry_run",
        "message": "Window construction recipe validated. No input table was loaded during the dry run.",
