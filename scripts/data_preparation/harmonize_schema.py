...
def fit_derived_state(df: pd.DataFrame) -> dict[str, object]:
    """Fit train-only derived-feature state from a prepared canonical frame."""
    state: dict[str, object] = {
        "port_frequency": {},
        "normalization": "not_applied_by_reference_recipe",
    }
    if "dst_port" in df.columns:
        counts = df["dst_port"].astype(str).value_counts(dropna=False)
        state["port_frequency"] = {str(k): int(v) for k, v in counts.items()}
    return state


def transform_with_derived_state(df: pd.DataFrame, state: dict[str, object]) -> pd.DataFrame:
    """Apply train-fitted derived-feature state without refitting on this frame."""
    out = df.copy()
    counts = state.get("port_frequency", {})
    if "port_rarity" not in out.columns:
        if "dst_port" in out.columns and isinstance(counts, dict):
            out["port_rarity"] = (
                out["dst_port"].astype(str)
                .map(lambda value: 1.0 / float(counts[value]) if value in counts and counts[value] else NUMERIC_SENTINEL)
                .astype(float)
            )
        else:
            out["port_rarity"] = NUMERIC_SENTINEL
    return out
...
def compute_derived(df: pd.DataFrame, state: dict[str, object] | None = None) -> pd.DataFrame:
    df = df.copy()
    ...
    if state is not None:
        df = transform_with_derived_state(df, state)
    elif "port_rarity" not in df.columns:
        df = transform_with_derived_state(df, fit_derived_state(df))
...
def harmonize_frame(df: pd.DataFrame, source_dataset: str, label_column: str | None) -> pd.DataFrame:
    """Harmonize one training or synthetic frame, fitting derived state on that frame.

    Validation and test data should use ``harmonize_frame_with_state`` with
    state fitted from training rows.
    """
    raw = df.copy()
...
def harmonize_frame_with_state(
    df: pd.DataFrame,
    source_dataset: str,
    ...
    raw.columns = make_unique_columns(raw.columns)
    mapped, source = prepare_base_frame(raw, source_dataset)
    fitted_state = fit_derived_state(mapped) if state is None else state
    mapped = compute_derived(mapped, fitted_state)
    label_values = choose_label_values(raw, mapped, source, label_column)
    mapped = drop_identifier_columns(mapped)
    out = select_model_features(mapped, load_feature_spec())
    if label_values is not None:
...
def run_self_test() -> dict[str, object]:
    ...
    train = pd.DataFrame({
        ...
        "sport": [1, 2, 3],
        "dsport": [80, 80, 443],
        "attack_cat": ["Normal", "DoS", "Reconnaissance"],
    })
    val_a = train.assign(dsport=[80, 22, 22])
    val_b = train.assign(dsport=[22, 22, 22])
    _, state_a = harmonize_frame_with_state(train, "UNSW", None)
    _, state_b = harmonize_frame_with_state(train, "UNSW", None)
    val_a_out, _ = harmonize_frame_with_state(val_a, "UNSW", None, state_a)
    val_b_out, _ = harmonize_frame_with_state(val_b, "UNSW", None, state_a)
    checks["train_fitted_port_state_stable"] = state_a == state_b
    checks["validation_values_do_not_refit_port_state"] = (
        val_a_out["port_rarity"].tolist() == [0.5, 0.0, 0.0]
        and val_b_out["port_rarity"].tolist() == [0.0, 0.0, 0.0]
        and state_a["port_frequency"] == {"80": 2, "443": 1}
    )
    result = {"status": "PASS" if all(checks.values()) else "FAIL", "checks": checks, "case_rows": {"cicids": int(len(cicids_out)), "unsw": int(len(unsw_out)), "mqcids": int(len(mqcids_out))}, "columns": int(len(cicids_out.columns))}
    OUTPUTS.mkdir(exist_ok=True)
    (OUTPUTS / "harmonize_schema_self_test.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    return result


def main() -> int:
    parser = argparse.ArgumentParser(description="Apply the schema and label harmonization recipe.")
    ...
    parser.add_argument(
        "--fit-state-output",
        type=Path,
        help="Fit derived-feature state on this training table and write the JSON state under outputs/.",
    )
    parser.add_argument(
        "--state-input",
        type=Path,
        help="Apply a previously fitted training-state JSON without refitting on this table.",
    )
    ...
    if args.input is None:
        ...
        OUTPUTS.mkdir(exist_ok=True)
        (OUTPUTS / "harmonize_schema_dry_run.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
        print(json.dumps(summary, indent=2))
        return 0

    if args.fit_state_output and args.state_input:
        raise SystemExit("Use either --fit-state-output for training data or --state-input for validation/test data, not both.")
    if not args.dry_run and not args.fit_state_output and not args.state_input:
        raise SystemExit(
            "Choose --fit-state-output when processing training data, or --state-input when applying training-fitted state to validation/test data."
        )

    df = read_table(args.input)
    state = json.loads(args.state_input.read_text(encoding="utf-8")) if args.state_input else None
    out, fitted_state = harmonize_frame_with_state(df, args.source_dataset, args.label_column, state)
    if args.dry_run:
        print(json.dumps({"status": "PASS", "input_rows": int(len(df)), "output_columns": list(out.columns)}, indent=2))
        return 0
    if args.fit_state_output:
        state_path = safe_output_path(args.fit_state_output)
        state_path.parent.mkdir(parents=True, exist_ok=True)
        state_path.write_text(json.dumps(fitted_state, indent=2) + "\n", encoding="utf-8")
