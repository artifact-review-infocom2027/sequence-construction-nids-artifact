...
from .utils import reject_full_run_without_assets
...
def recipe_summary(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "note": "Dry run only. Full-scale training requires external data and model assets.",
    }
...
def train_from_config(cfg: dict[str, Any], *, dry_run: bool = False) -> dict[str, Any]:
    ...
    reject_full_run_without_assets(cfg, synthetic_smoke=False)
    require_transformers_available()
    raise RuntimeError(
        "Full-scale LLM fine-tuning is outside this release. "
        "Use the dry-run recipe summary here and the original training pipeline with external data and model assets."
    )
