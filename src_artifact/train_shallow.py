...
from .utils import ensure_dir, reject_full_run_without_assets, save_json, seed_everything
...
def train_from_config(cfg: dict[str, Any], *, synthetic_smoke: bool = False) -> dict[str, Any]:
    reject_full_run_without_assets(cfg, synthetic_smoke=synthetic_smoke)
    output_dir = cfg.get("paths", {}).get("output_dir", "outputs/shallow_recipe")
    if synthetic_smoke:
        return run_synthetic_smoke(cfg, output_dir)
    raise RuntimeError(
        "Full-scale shallow training is outside this release. "
        "Use the synthetic smoke test here, or supply external data to the original training pipeline."
    )
