...
from .utils import contains_external_marker
...
def serialize_flow_text_style(row: dict[str, Any]) -> str:
    ...
    or the complete feature set used in the reported experiments.
    """
    tokens: list[str] = []
...
def load_jsonl_text_label(path: str | Path) -> list[dict[str, Any]]:
    p = Path(path)
    if contains_external_marker(p):
        raise FileNotFoundError("External input tables are not included in this repository.")
    if not p.exists():
