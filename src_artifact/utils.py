...
def contains_external_marker(value: Any) -> bool:
    if value is None:
        return False
    text = str(value)
    return any(marker in text for marker in EXTERNAL_ASSET_MARKERS)
...
def validate_common_config(cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, value in paths.items():
        if isinstance(value, str) and not contains_external_marker(value):
            if not path_is_safe_relative(value):
                raise ValueError(f"Path must be relative or use an external-asset marker: paths.{key}={value}")

    return {
        "family": family,
        "has_external_markers": any(contains_external_marker(v) for v in walk_values(cfg)),
...
def reject_full_run_without_assets(cfg: Mapping[str, Any], synthetic_smoke: bool) -> None:
    info = validate_common_config(cfg)
    if synthetic_smoke:
        return
    if info["has_external_markers"] or info["requires_raw_data_not_included"]:
        raise ValueError(
            "Full-scale training cannot run while external-asset markers are present. "
